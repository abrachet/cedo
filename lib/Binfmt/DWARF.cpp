...
std::optional<DWARF::DIE> DWARF::getTypeDieFromVarDIE(const DIE &varDie) const {
  auto typeOffsetOrErr = varDie.getAttributeIfPresent(DW_AT_type);
  if (!typeOffsetOrErr)
    return {};
  ...
  auto it =
      std::find_if(debugInfo.begin(), debugInfo.end(),
                   [offset](const DIE &die) { return die.offset == offset; });
...
Type DWARF::getTypeFromTypeDIE(const DIE &typeDie) const {
  // TODO check die.tag is DW_TAG_structure_type, DW_TAG_base_type, etc
  auto attrOrErr = typeDie.getAttributeIfPresent(DW_AT_byte_size);
  // TODO error handling
  assert(attrOrErr &&
         "Currently all type die's must have a DW_AT_byte_size attribute");
...
std::optional<Type> DWARF::getVariableType(std::string_view sym_name) const {
  ...
  std::optional<DIE> typeDie = getTypeDieFromVarDIE(*it);
  if (!typeDie)
    return {};
